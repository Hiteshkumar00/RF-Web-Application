...
@Component({
    ...
    standalone: false,
    templateUrl: './remove-contribution-form-dialog.component.html'
})
export class RemoveContributionFormDialogComponent implements OnChanges {
    ...
    onSubmit(): void {
        ...
        if (this.mode === 'create') {
            delete payload.id;
            this.apiService.create(payload as CreateRemoveContributionDto).subscribe({
                next: () => this.onSave.emit()
            });
        } else {
            this.apiService.update({ id: this.id!, ...payload } as UpdateRemoveContributionDto).subscribe({
                next: () => this.onSave.emit()
            });
        }
    }
